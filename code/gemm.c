        ...
        float *C, int ldc)
{
    ...
    for(i = 0; i < M; ++i){ //此时M=1
        for(k = 0; k < K; ++k){ //卷积核大小
            ...
            register float A_PART = ALPHA*A[i*lda+k];//卷积核拉成向量 每个点
            for(j = 0; j < N; ++j){ //输出大小 相乘 相加
                C[i*ldc+j] += A_PART*B[k*ldb+j]; //B是指针 指向工作空间 是个矩阵
            }
        }
    }
}
        ...
        float *C, int ldc)
{
    int i,j,k;
    for(i = 0; i < M; ++i){ //M=1
        for(j = 0; j < N; ++j){ //output长度
            register float sum = 0;
            for(k = 0; k < K; ++k){ //input长度
                sum += ALPHA*A[i*lda+k]*B[j*ldb + k];//全部输入权重相加
            }
            C[i*ldc+j] += sum;
        }
    }
        ...
        float *C, int ldc)
{
    ...
    int i, j;
    for(i = 0; i < M; ++i){// 卷积核个数（conv） 或者是batch(fully connected)
        for(j = 0; j < N; ++j){ // 输出大小
            C[i*ldc + j] *= BETA; // output 每个乘以beta
        }
    }
