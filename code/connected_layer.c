...
void forward_connected_layer(connected_layer l, network_state state)
{
    ...
    float *b = l.weights;
    float *c = l.output;
    gemm(0,1,m,n,k,1,a,k,b,k,1,c,n); //输入和权重乘
    if(l.batch_normalize){
        if(state.train){
...
void backward_connected_layer(connected_layer l, network_state state)
{
    ...
    for(i = 0; i < l.batch; ++i){
        axpy_cpu(l.outputs, 1, l.delta + i*l.outputs, 1, l.bias_updates, 1);//更新偏置bias
    }
    if(l.batch_normalize){
    ...
    gemm(1,0,m,n,k,1,a,m,b,n,1,c,n);//更新权重
