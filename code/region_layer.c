...
{
    box b;
    b.x = (i + logistic_activate(x[index + 0])) / w;//对应论文中公式
    b.y = (j + logistic_activate(x[index + 1])) / h;//同上
...
{
    int i,j,b,t,n;
    int size = l.coords + l.classes + 1; //每个锚点的大小
    memcpy(l.output, state.input, l.outputs*l.batch*sizeof(float));
    #ifndef GPU
    flatten(l.output, l.w*l.h, size*l.n, l.batch, 1); //拉平
    #endif
    for (b = 0; b < l.batch; ++b){ //对每个batch
        for(i = 0; i < l.h*l.w*l.n; ++i){//对每个锚点
            int index = size*i + b*l.outputs;//索引
            l.output[index + 4] = logistic_activate(l.output[index + 4]);//置信度logistic 归一到0-1
        }
    }
    ...
    if (l.softmax_tree){
        ...
        for (b = 0; b < l.batch; ++b){
            for(i = 0; i < l.h*l.w*l.n; ++i){
                int index = size*i + b*l.outputs;
                softmax(l.output + index + 5, l.classes, 1, l.output + index + 5);//类别 softmax
            }
        }
    }
#endif
    if(!state.train) return;
    memset(l.delta, 0, l.outputs * l.batch * sizeof(float)); //全部置零 存储损失函数
    float avg_iou = 0;
    float recall = 0;
    float avg_cat = 0;
    float avg_obj = 0;
    ...
    *(l.cost) = 0;
    for (b = 0; b < l.batch; ++b) {
        ...
        for (j = 0; j < l.h; ++j) {
            for (i = 0; i < l.w; ++i) {
                for (n = 0; n < l.n; ++n) {//每个锚点
                    int index = size*(j*l.w*l.n + i*l.n + n) + b*l.outputs;//计算预测的框的位置
                    box pred = get_region_box(l.output, l.biases, n, index, i, j, l.w, l.h);//框
                    float best_iou = 0;
                    int best_class = -1;
                    for(t = 0; t < 30; ++t){//每个锚点 固定扫描该图中30个真实框 看与的IOU
                        box truth = float_to_box(state.truth + t*5 + b*l.truths);//
                        if(!truth.x) break;
                        float iou = box_iou(pred, truth);//计算当前预测框与真实框的IOU
                        if (iou > best_iou) {//寻找最优的
                            best_class = state.truth[t*5 + b*l.truths + 4];
                            best_iou = iou;
                        }
                    }
                    avg_anyobj += l.output[index + 4];
                    l.delta[index + 4] = l.noobject_scale * ((0 - l.output[index + 4]) * logistic_gradient(l.output[index + 4]));//置信度 假定无物体
					//如果当前预测框确实没有物体（背景） 但是与别的真实框有交集 置信度的损失函数为
                    if(l.classfix == -1) l.delta[index + 4] = l.noobject_scale * ((best_iou - l.output[index + 4]) * logistic_gradient(l.output[index + 4]));
                    else{//如果当前IOU大于设定阈值 置信度无损失
                        if (best_iou > l.thresh) {//
                            l.delta[index + 4] = 0;//如果检测大于阈值 无损失
                            if(l.classfix > 0){//分类损失
                    ...
                    if(*(state.net.seen) < 12800){//开始的训练时 计算框的四个点的偏差损失
                        box truth = {0};
                        truth.x = (i + .5)/l.w;//cell的中点
                        truth.y = (j + .5)/l.h;
        ...
        for(t = 0; t < 30; ++t){
            ...
            truth_shift.y = 0;
            //printf("index %d %d\n",i, j);
            for(n = 0; n < l.n; ++n){//找到真实物体所在的cell的最优预测框
